...
const PHASE_MIN_EXCHANGES = {
  ...
  1: 3,  // Focus & Attention - reduced for better flow
  2: 3,  // Energy & Movement
  3: 3,  // Impulse Control
  4: 3,  // Emotions - reduced for better flow
  5: 2   // Wrap-up
};

/**
 * Maximum exchanges per phase before forced transition (prevents getting stuck)
 */
const PHASE_MAX_EXCHANGES = {
  0: 4,  // Introduction
  1: 6,  // Focus & Attention
  2: 6,  // Energy & Movement
  3: 5,  // Impulse Control
 ...
 * REDUCED: Allow transitions based on engagement, not just DSM-5 keyword detection
 */
const PHASE_MIN_INDICATORS = {
  0: 0,  // Introduction - no indicators needed
  1: 0,  // Focus & Attention - removed strict requirement (user engagement counts)
  2: 0,  // Energy & Movement - removed strict requirement
  3: 0,  // Impulse Control - removed strict requirement
  4: 0,  // Emotions - removed strict requirement
...
3. INQUIRY: Ask ONE new question that explores a DIFFERENT aspect of ${phase.name}. Move the conversation forward. (1 question)

CRITICAL RULES:
- DO NOT repeat the same question if the user already answered it
- DO NOT ask "can you elaborate?" - they already gave you information
- ACCEPT short answers like "yes", "no", "always", "sometimes" and MOVE ON to a new question
- If user gave you ANY answer (even brief), acknowledge it and ask about a DIFFERENT behavior or situation
...
MOVING FORWARD EXAMPLES:
- After asking about sitting still → Ask about fidgeting with hands/objects
- After asking about homework → Ask about chores or following instructions
- After asking about interrupting → Ask about waiting for turn
- After asking about losing things → Ask about forgetfulness

EXAMPLE GOOD RESPONSE:
"Thanks for sharing that - that constant need to move is really common. Now I'm curious about a different area: when ${name} is working on something, like homework or a project, how does ${name} handle distractions?"

EXAMPLE BAD RESPONSE (DO NOT DO THIS):
"I see how that could make things tough. When you feel the need to move, do you try to stay seated and fidget quietly, or does it sometimes become too strong so you have to get up?" [This is repeating the same question!]`;
  }
  ...
  static async generate(userMessage, conversation, responseTime) {
    ...
    const maxExchanges = PHASE_MAX_EXCHANGES[currentPhase] || 6;
    const minIndicators = PHASE_MIN_INDICATORS[currentPhase] || 0;
    
    // Allow advancement if:
    // 1. Met minimum exchanges AND minimum indicators, OR
    // 2. Exceeded maximum exchanges (prevents getting stuck)
    const metMinimumRequirements = phaseExchanges >= minExchanges && phaseIndicatorCount >= minIndicators;
    const exceededMaxExchanges = phaseExchanges >= maxExchanges;
    const canAdvance = (metMinimumRequirements || exceededMaxExchanges) && currentPhase < PHASES.length - 1;
    
    console.log(`[AdaptiveResponseGenerator] Phase transition check: ` +
      `exchanges=${phaseExchanges}/${minExchanges}(max:${maxExchanges}), ` +
      `indicators=${phaseIndicatorCount}/${minIndicators}, ` +
      `canAdvance=${canAdvance}${exceededMaxExchanges ? ' (forced by max exchanges)' : ''}`);
